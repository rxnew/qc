...
auto Json::Dumper::dump() -> void {
  auto json = _make_json("qc", circuit_);
  os_ << json.dump() << std::endl;
}

auto Json::Dumper::_make_json(std::string const& format,
                              Circuit const& circuit) -> json11::Json {
  return json11::Json::object{
    {"format" , format},
    {"circuit", _make_json(circuit)}
  };
}
...
auto Json::Dumper::_make_json(Circuit const& circuit) -> json11::Json {
  ...
  return json11::Json::object{{"gates", json_array}};
}
