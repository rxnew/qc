...
auto Gate::get_cbit(BitNo bit_no) const -> CBit const& {
  auto cbit_pos = kernel_->get_cbits().find(CBit(bit_no, true));
  if(cbit_pos != kernel_->get_cbits().cend()) return *cbit_pos;
  cbit_pos = kernel_->get_cbits().find(CBit(bit_no, false));
  assert(cbit_pos != kernel_->get_cbits().cend());
  return *cbit_pos;
}

auto Gate::get_tbit(BitNo bit_no) const -> TBit const& {
  const auto tbit_pos = kernel_->get_tbits().find(TBit(bit_no));
  assert(tbit_pos != kernel_->get_tbits().cend());
  return *tbit_pos;
}

auto Gate::invert_cbit_polarity(BitNo bit_no) -> bool {
  auto polarity = get_cbit_polarity(bit_no);
  kernel_->get_cbits().erase(CBit(bit_no, polarity));
...
auto Gate::collect_bits() const -> BitNos {
  auto bit_nos = BitNos();
  for(auto const& cbit : kernel_->get_cbits()) {
    bit_nos.insert(cbit.get_no());
  }
  for(auto const& tbit : kernel_->get_tbits()) {
